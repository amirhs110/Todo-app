...
class TestAccountsRegistration:
    ...
    def test_registration_user_invalid_data_response_400_status(self, api_client):
        url = reverse("accounts:registration")
        data = {"email": "test@test.com", "password": "123", "re_password": "123"}
        response = api_client.post(url, data, format="json")
        assert response.status_code == 400

    def test_activation_user_response_200_status(
        self, api_client, user_common, jwt_access_token
    ):
    ...
    def test_activation_user_invalid_token_response_400_status(
        self, api_client, user_common, jwt_access_token
    ):
        # Generate the activation token
        token = jwt_access_token
